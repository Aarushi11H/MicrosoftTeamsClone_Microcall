...
let API_KEY_SECRET = process.env.API_KEY_SECRET || "microcall_default_secret";
...
app.post(["/api/v1/meeting"], (req, res) => {
  ...
  if (authorization != API_KEY_SECRET) {
    logme("Microcall get meeting - Unauthorized", {
      header: req.headers,
      body: req.body,
    });
    return res.status(403).json({ error: "Unauthorized!" });
  }
  // setup Microcall meeting URL
  ...
  logme("Microcall get meeting - Authorized", {
    header: req.headers,
    body: req.body,
    meeting: meetingURL,
  });
