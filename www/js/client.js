...
let recStartTime;
let recElapsedTime;
let microcallTheme = "default"; // default - dark - forest - ghost ...
...
function getHtmlElementsById() {
  ...
  themeSelect = getId("microcallTheme");
  ...
  function welcomeUser() {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        let message = {
          email: "",
          subject: "Please join our microcall Video Chat Meeting",
        ...
        event.channel.onmessage = (msg) => {
          switch (event.channel.label) {
            case "microcall_chat_channel":
              ...
              break;
            case "microcall_file_sharing_channel":
...
/**
 * Set microcall theme default - dark - forest - sky - ghost
 * @param {*} theme
 */
function setTheme(theme) {
  if (!theme) return;

  microcallTheme = theme;
  switch (microcallTheme) {
    case "default":
      // default theme
      ...
      document.documentElement.style.setProperty(
        "--box-shadow",
        "3px 3px 6px rgba(0,85,67,1), -3px -3px 6px rgba(60,186,151,1)"
      );
      break;
    case "dark":
      // dark theme
...
function setChatRoomBtn() {
  ...
  msgerTheme.addEventListener("click", (e) => {
    if (microcallTheme == "ghost") return;
...
function createFileSharingDataChannel(peer_id) {
  fileSharingDataChannels[peer_id] = peerConnections[peer_id].createDataChannel(
    "microcall_file_sharing_channel"
  );
...
function selectFileToShare() {
  ...
  Swal.fire({
    allowOutsideClick: false,
    background: swalBackground,
    imageAlt: "microcall-file-sharing",
  ...
  if (isImageURL(incomingFileInfo.fileName)) {
    ...
    reader.onload = (e) => {
      Swal.fire({
        ...
        imageAlt: "microcall-file-img-download",
        showDenyButton: true,
    ...
    Swal.fire({
      allowOutsideClick: false,
      background: swalBackground,
      imageAlt: "microcall-file-download",
      imageUrl: fileSharingImg,
      position: "center",
      title: "Received file",
      text:
...
function disableAllPeers(element) {
  ...
  Swal.fire({
    background: swalBackground,
    position: "center",
    imageAlt: "microcall-disable-" + element,
    imageUrl: confirmImg,
    title:
      element == "audio"
        ? "Mute everyone except yourself?"
...
function getAbout() {
  ...
  Swal.fire({
    background: swalBackground,
    position: "center",
    title: "<strong>WebRTC </strong>",
    imageAlt: "microcall-about",
    imageUrl: aboutImg,
    html: `
    <br/>
    <div id="about"><b>Microsoft Engage Project</b> <a href="https://github.com/Aarushi11H/MicroTeamsSoft" target="_blank"><h1><strong> GitHub </strong></h1></a></div>
    <div id="author"><a href="https://www.linkedin.com/in/aarushihans/" target="_blank">By: Aarushi Hans</a></div><br>
